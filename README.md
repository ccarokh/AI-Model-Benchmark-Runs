...
| **How many users per card** | **[Serving many users](findings/serving-many-users.md)** | **The user ceiling is the context ceiling divided by the per-user budget — but only for one slot each. 32 people on 4 slots, nobody refused: the queue costs waiting, and coming back after eviction costs 1–17 % of a cold start** |
