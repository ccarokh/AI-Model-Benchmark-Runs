...
| **RAG — which language** | **[Language gap](findings/language-gap.md)** | **All seven models read English best. The penalty for German runs from 0.7 points to 8.0 on identical questions, so it is the models and not the translation — and one model is second in English and fifth in German. Asking in English does not rescue a German passage: identical to four decimals** |
...
| **How many users per card** | **[Serving many users](findings/serving-many-users.md)** | **On 12 GB the memory refuses first; on 24 GB it holds and the card collapses anyway — 64 users, no failures, 0.35 tokens/s each. Sixteen is the usable number on both. 32 people on 4 slots are served fine: the queue costs waiting, and coming back after eviction costs 1–17 % of a cold start** |
...
| **Qwen3.8-Flash-Next** — 177B, the first open-weight release of the Qwen4 architecture, and the next step of the exact line this repository has been tracking: Gated DeltaNet paired with Qwen Sparse Attention. [Kimi-Linear lost 87.9 % of prefill at depth](findings/context-depth.md) and [the hybrid 27B lost 30.3 %, matching the dense model it replaced](findings/context-depth.md#qwen38-27b-a-hybrid-that-behaves-exactly-like-the-dense-model-it-replaces) — QSA is the third data point and the most interesting one on the list. **The smallest quantisation that exists is 67.6 GB** (UD-IQ1_S; Q4 is 103.7 GB). Against the [measured ceiling](#size-ceiling) of 24 GB that is a factor of 2.8, and against the 32 GB both cards could hold together — [capacity is what the second card buys](hardware/multi-gpu.md) — still 2.1. Anything beyond that would have to come out of host memory, which has never been measured here for a model of any size and is 15 GB in total. Its `qwen4_exp` architecture also landed in llama.cpp master on 2026-08-27, after the pinned build. Not served by the [foreign endpoint](scripts/foreign/) either, checked 2026-09-02 | **cannot be run here** — watch for it appearing on a hosted endpoint, which is the only route that does not need different hardware |
...
| **Vulkan tensor split — upstream contribution.** `-sm tensor` on Vulkan runs at 34 % of a single card here; `ggml-vulkan.cpp` has no AllReduce of its own and falls through to the generic path in `ggml-backend-meta.cpp`, which the tracking issue [#22648](https://github.com/ggml-org/llama.cpp/issues/22648) calls "crazy slow". The cause is diagnosed upstream and unassigned ("someone needs to go work on it"). CUDA's native version (`allreduce.cu`, 971 lines) is the template. This repository has what the fix needs and the issue lacks: a real dual-card machine, a measurement that resolves 1 %, and a nightly regression row (`mgpu-tensor`). A working Vulkan path would also bring cheap high-bandwidth cards back into play that ROCm has dropped | **idea recorded, prompt written, not started** |
