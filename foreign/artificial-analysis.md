...
**So the ceiling is not simply the card** — but nor is it the sum of everything present. The largest VRAM peak ever recorded here is 24.0 GB, exactly one card; the second card adds capacity at a measured cost in speed; and host memory has never been measured as a path for a large model at all. 24 GB measured, 32 GB plausible, anything above that unknown. It changes none of the rows above, because nothing on that chart lands between 24 and 39 GB; the next entry up is Flash-Next at 119.6 GB.
