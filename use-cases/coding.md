...
**Stage 1 — the same run on a build with the GDN fix (2026-09-21):** upstream merged a correction to the gated-delta-net normalisation on 06.09. (#28068, unsloth: `x / max(sqrt(sum x²), eps)` → `x · rsqrt(sum x² + eps)`), which touches every Qwen3.5/3.6/3.8 model. The full 225 were run again on a pinned master snapshot from 17.09. (`972d2313b`), everything else identical, no timeouts, median 11.3 min per task: **pass@1 28.0 %, pass@2 66.7 %** against 25.8 / 68.9 on v0.2.0. Within noise both ways, per language ±2 tasks. **The build is not the reason** Qwen3.8-27B trails Qwen3.6-27B here. It writes even more on master: 13 200 completion tokens per task against 11 600.

**Stage 0 — what Q4_K_M costs this model (2026-09-20):** KL divergence against BF16 on 45 000 tokens of private, never-published Rust and Python (BF16 perplexity 3.10, so genuinely unseen), same pinned build. Q8_0 0.0013 mean KLD and 98.5 % same top token; UD-Q6_K 0.0022 / 98.1 %; **Q4_K_M 0.0092 / 96.4 %** — Q4 changes the most likely token on every 28th token, Q6 on every 54th. On a memorised text (llama.cpp's own source, perplexity 1.32) the same Q4 file showed 98.7 %; the number that matters is the unseen one. Data: [`data/kld_qwen38_27b.tsv`](../data/kld_qwen38_27b.tsv). Whether 3.6 % altered tokens account for the gap is the open stage-2 question — a polyglot run on Q6_K, which needs two slots instead of four to fit beside the 32k context.
